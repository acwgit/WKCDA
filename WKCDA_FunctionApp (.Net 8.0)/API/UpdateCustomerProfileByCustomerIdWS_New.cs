...
public class UpdateCustomerProfileByCustomerIdWS_New : WS_Base
{
    public UpdateCustomerProfileByCustomerIdWS_New(ILogger<UpdateCustomerProfileByCustomerIdWS> logger) : base(logger)
    {
    }

    [Function("UpdateCustomerProfileByCustomerIdWS_New")]
    public async Task<IActionResult> RunAsync([HttpTrigger(AuthorizationLevel.Function, "post", Route = "data/v9.2/UpdateCustomerProfileByCustomerIdWS_New")] HttpRequest req)
    {
        _logger.LogInformation("APIName: UpdateCustomerProfileByCustomerIdWS_New");
        ...
        try
        {
            ...
            foreach (var c in requestBody.customerList)
            {
                ...
                #region Validations
                {
                    ...
                    {
                        resultItem.Remarks = $"{c.MasterCustomerID}:Master Customer ID does not exist.";
                        resultItem.MasterCustomerID = c.MasterCustomerID;
                        isValid = false;
                        continue;
                    }
                    else
                    ...
                    if (string.IsNullOrEmpty(c.Email) ||
                        !System.Text.RegularExpressions.Regex.IsMatch(c.Email, @"^[^@\s]+@[^@\s]+\.[^@\s]+$"))
                    {
                        resultItem.Remarks = $"Invalid email address: {c.Email}";
                        resultItem.MasterCustomerID = c.MasterCustomerID;
                ...
                try
                {
                    ...
                    else if (c.InterestStep == "Update")
                    {
                        //if (oldInterests.size() > 0) { e.CustomerInterestSelected__pc = String.join(oldInterests, ';'); } else { e.CustomerInterestSelected__pc = ''; }
                        //if (interests.size() > 0) { e.CustomerInterest2__pc = String.join(interests, ';'); } else { e.CustomerInterest2__pc = ''; }
                        //if (artForms.size() > 0) { e.CustomerInterest1__pc = String.join(artForms, ';'); } else { e.CustomerInterest1__pc = ''; }
                    }
                    if (!string.IsNullOrEmpty(c.MonthofBirth))
                    {
                        e.WKCDA_MonthOfBirth = c.MonthofBirth;
                    }
                    if (!string.IsNullOrEmpty(c.AltCntcFirstName))
                        e.WKCDA_AlternateContactPersonFirstName = c.AltCntcFirstName;
                    if (!string.IsNullOrEmpty(c.AltCntcLastName))
                        e.WKCDA_AlternateContactPersonLastName = c.AltCntcLastName;
                    if (!string.IsNullOrEmpty(c.AltCntcEmail))
                        e.WKCDA_AlternateContactPersonalEmail = c.AltCntcEmail;
                    if (!string.IsNullOrEmpty(c.AltCntcMobileCountryCode))
                        e.WKCDA_AlternateContactMobileCountryCode = GetCountryRef(ecCountry, c.AltCntcMobileCountryCode);
                    if (!string.IsNullOrEmpty(c.AltCntcMobile))
                    ...
                    //customer.IsSubsidaryAccount
                    // customer.NeedEligibilityCheck
            ...
            if (ecContactToCreate.Entities.Count > 0)
            {
                ...
                {
                    _logger.LogInformation("updateMultipleRequest_new : " + ex.InnerException.ToString());
                    throw;
                }
