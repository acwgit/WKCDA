...
{
    public class RetrieveMemberInfoByEmailWS : WS_Base
    {
        public RetrieveMemberInfoByEmailWS(ILogger<RetrieveMemberInfoByEmailWS> logger) : base(logger) { }

        [Function("RetrieveMemberInfoByEmailWS")]
        public async Task<IActionResult> RunAsync([HttpTrigger(AuthorizationLevel.Function, "post", Route = "data/v9.2/RetrieveMemberInfoByEmailWS")] HttpRequest req)
        {
            _logger.LogInformation("APIName: RetrieveMemberInfoByEmailWS");
            ...
            try
            {
                ...
                var contact = GetContactByEmail(svc, customer.Email);
                if (contact == null)
                {
                    var errorResponse = new
                    {
                        Success = false,
                        Remarks = $"{customer.Email}: Customer does not exist.",
            ...
            catch (Exception ex)
            {
                ...
                {
                    _logger.LogError(ex, "Error in RetrieveMemberInfoByEmailWS: {Message}", ex.Message);
                    return new ObjectResult(new
                    {
                        error = "Bad Request",
                        message = ex.Message
