...
{
    public class VoucherValidate : WS_Base
    {
        public VoucherValidate(ILogger<VoucherValidate> logger) : base(logger) { }

        [Function("VoucherValidate")]
        public async Task<IActionResult> RunAsync(
            [HttpTrigger(AuthorizationLevel.Function, "post", Route = "data/v9.2/VoucherValidate")] HttpRequest req)
        {
            _logger.LogInformation("APIName: VoucherValidate");
            ...
            try
            {
                ...
                var responses = new List<ResponseBody>();
                ...
                foreach (var voucher in requestBody.voucherInfos)
                {
                    ...
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(voucher.VoucherCode) && string.IsNullOrEmpty(voucher.VoucherSFID))
                        {
                            response.Remarks = "Voucher Code and SFID cannot be both empty";
                        ...
                        else if (!string.IsNullOrEmpty(voucher.VoucherSFID))
                            qe.Criteria.AddCondition("wkcda_referenceno", ConditionOperator.Equal, voucher.VoucherSFID);
                        ...
                        var codeStatus = voucherEntity.GetAttributeValue<string>("wkcda_codestatus") ?? "Activated";

                        // 1. Already Consumed
                        if (!string.IsNullOrEmpty(codeStatus) && codeStatus.Equals("Consumed", StringComparison.OrdinalIgnoreCase))
                        {
                            response.Remarks = "Voucher Code Used";
                        ...
                        if (!string.IsNullOrEmpty(codeStatus) && codeStatus.Equals("Expired", StringComparison.OrdinalIgnoreCase))
                        {
                        ...
                        response.StartDate = voucherEntity.GetAttributeValue<DateTime?>("wkcda_startdate")?.ToString("yyyy-MM-dd");
                        response.EndDate = voucherEntity.GetAttributeValue<DateTime?>("wkcda_enddate")?.ToString("yyyy-MM-dd");
                        ...
                        response.CodeStatus = voucherEntity.GetAttributeValue<string>("wkcda_codestatus");
                        ...
                        response.LatestActivationDate = voucherEntity.GetAttributeValue<DateTime?>("wkcda_latestactivationdate")?.ToString("dd-MM-yy");
                        response.UsedTimes = voucherEntity.GetAttributeValue<double>("wkcda_usedtimes");

                        // Membership Tier History and Contact
                        var tierRef = voucherEntity.GetAttributeValue<EntityReference>("wkcda_membershiptierhistory");
                        if (tierRef != null)
                        {
                            response.MembershipTierHistoryId = tierRef.Id.ToString();
                        ...
                        #endregion
                        wrapper.lstVoucherResult.Add(response);
                        wrapper.isSuccess = false; // if failed
